...
def recreate_knob_from_optimizer_values(knobs_and_results):
    ...
    knobs_and_results = knobs_and_results.sort(key=itemgetter(1))
    # sorted_knobs_by_results = sorted(knobs_and_results, key=lambda tup: tup[1])
    return knobs_and_results[0][0] # 0: knob_object, 1: value
