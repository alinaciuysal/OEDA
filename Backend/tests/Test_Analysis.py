...
from tests.Test_Unit import UnitTest
from tests.Test_Integration import IntegrationTest
from tests.integration_test_utility import parse_config
...
''' Simulation systems should be running in the background
    ...
    main method contains a test suite that includes
        1) unit_test
        2) integration_test
        3) analysis test
    
    a random experiment and its stages are sampled for different statistical analysis tests
...
class AnalysisTest(unittest.TestCase):
    ...
    outer_key = "payload"
    key = "overhead"
    mean_diff = 0.1 # as in rtx/crowdnav-elastic-ttest-sample-size/definition.py
    # n = number of stages to be used for analysis
    # if n > 2, two-sample tests only use the first two stages,
    # and FactorialAnova generally generates SingularMatrix error
    n = 2
    ...
    def test_s_FactorialAnova(self):
        try:
            stage_ids, samples, knobs = AnalysisTest.get_data_for_tests()
            test = FactorialAnova(y_key=AnalysisTest.key, knob_keys=None, stages_count=len(stage_ids))
            result = test.run(data=samples, knobs=knobs)
